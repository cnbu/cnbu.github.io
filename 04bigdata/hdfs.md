...
#查看根目录文件
sudo -u hdfs hdfs dfs -ls / 　　　　　　 #hadoop fs -ls / 　　　　　　  

#hdfs上传文件，-f 覆盖已有文件；-p 保留原文件属性
hdfs dfs -put sourcepath savepath 　　　　　　　　　

#hdfs下载文件
hdfs dfs -get sourcepath savepath 　　　　　　　　　　 

#创建文件夹dirname
hdfs dfs -mkdir -p /output/dirname 　　　　　　　　

#授权
sudo -u hdfs hdfs dfs -chown elastic:elastic /xiebao_recall /huize_recall /qixin_recall /jumi_recall

#创建多级目录
hdfs dfs -mkdir -p /output/dirname 　　　　　　　　　

#创建文件filename
hdfs dfs -touchz /input/filename 　　　　　　　　　

#查看文件filename
hdfs dfs -cat /input/filename 　　　　　　　　
...
#删除文件
hdfs dfs -rm /directory/ 

#删除文件夹，-R循环删除文件夹里面的文件　　　　
hdfs dfs -rm -R /directory/ 　　　　　　　　　　　　

#找回删除文件
hdfs dfs -mv /user/root/.Trash/Current/test/filename /test/filename 　　　　　　　
...
#合并文件
hdfs dfs -getmerge sourcepaths savefilename

#查看HDFS的基本统计信息 　　　　　　　
hdfs dfsadmin -report 　　　　　　　　　　　　　　　　　

#将给定的文件以文本的格式输出,zip、TextRecordInputStream、Avro
hdfs dfs -text /input/log.txt 　　　　　　　　　　　　　　　

#向目标文件中追加内容
hdfs dfs -appendToFile /sourcefile /targetfile 　　　　　

#统计文件夹和文件个数
hdfs dfs -count /input/test/ 　　　　　　　　　　　　　　　

#查看文件分区存储情况
hdfs fsck /data/test.csv -files -racks -locations -blocks 　　　 

#动态查看log日志
tail -f filename.log 　

#创建目录
sudo -u hdfs hdfs dfs -mkdir -p /xiebao_recall /huize_recall /qixin_recall /jumi_recall
#授权
sudo -u hdfs hdfs dfs -chown elasticsearch:elasticsearch /xiebao_recall /huize_recall /qixin_recall /jumi_recall
```
...
```shell
[root@cdh002 beidou_flow_t_ip_area_parquet]# hdfs dfs -ls /user/hive/warehouse/bd_tmp.db/beidou_flow_t_ip_area_bak_parquet
[root@cdh002 beidou_flow_t_ip_area_parquet]# hdfs dfs -ls /user/hive/warehouse/bd_tmp.db/
Found 1 items
drwxrwxr-t   - hdfs hive          0 2021-09-28 18:06 /user/hive/warehouse/bd_tmp.db/beidou_flow_t_ip_area_bak_parquet
[root@cdh002 beidou_flow_t_ip_area_parquet]# sudo -u hdfs hdfs dfs -put 000000_0 /user/hive/warehouse/bd_tmp.db/beidou_flow_t_ip_area_bak_parquet
put: `000000_0': No such file or directory

解决办法，/etc/profile添加环境变量
export HADOOP_USER_NAME=hdfs
```
...
##### 3、DataNode：
 ...
    就是Slave。NameNode 下达命令，DataNode 执行实际的操作。储实际的数据块。
...
![](https://img-blog.csdn.net/20180716221908696?watermark/2/text/aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3doZHhqYnc=/font/5a6L5L2T/fontsize/400/fill/I0JBQkFCMA==/dissolve/70#id=wa72C&originHeight=291&originWidth=628&originalType=binary&ratio=1&rotation=0&showTitle=false&status=done&style=none&title=)
...
#### 4.3.4：SecondaryNameNode 如何辅助管理 fsimage 与 edits 文件?
