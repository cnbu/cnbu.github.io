...
```
Hive 中默认有三个文件格式 TextFile，SequenceFile 以及 RCFile parquetfile
hive 只适合用来做批量数据统计分析
Hive 的基本数据类型有： TINYINT，SAMLLINT，INT，BIGINT，BOOLEAN，FLOAT，DOUBLE，STRING，TIMESTAMP(V0.8.0+)和 BINARY(V0.8.0+)
Hive 的集合类型有： STRUCT，MAP 和 ARRAY 
Hive 主要有四种数据模型(即表)：内部表、外部表、分区表和桶表
hive 只支持 y Derby 和 和 L MySQL  数据库
Hive 支持的存储数的格式主要有：TEXTFILE（行式存储） 、SEQUENCEFILE(行式存储)、ORC（列式存储）、PARQUET（列式存储）
存储文件的查询效率比较: ORC > >  TextFile > >  Parquet
发生数据倾斜的原因有两种 ： 一是任务中需要处理大量相同的 的 y key  的数据。二是任务读取不可分割的大文件
```
...
这里我们选用hive的版本是2.1.1
下载地址为：
[http://archive.apache.org/dist/hive/hive-2.1.1/apache-hive-2.1.1-bin.tar.gz](http://archive.apache.org/dist/hive/hive-2.1.1/apache-hive-2.1.1-bin.tar.gz)
...
[多分区表联合查询(使用 ](/data/20180101/1.log)`[union all](/data/20180101/1.log)`[)](/data/20180101/1.log)
...
![](http://ppw6n93dt.bkt.clouddn.com/8dda7bfdfab0655e99a3c3b17afc422e.png#id=Jbzub&originalType=binary&ratio=1&rotation=0&showTitle=false&status=done&style=none&title=)
...
官网：[https://cwiki.apache.org/confluence/display/Hive/LanguageManual+ORC](https://cwiki.apache.org/confluence/display/Hive/LanguageManual+ORC)
